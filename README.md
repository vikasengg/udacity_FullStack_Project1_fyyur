...
## Steps Done:
Step 1: Basic Execeution without modifying the code: 
- Udpated the requirements.txt files to remove the versions related to each package. This helped to resolve the compilation errors I was getting. 
- Created the fyyur database in the postgres SQL Server.
- Update the config file to provide the database connection details.
- Set the python virtual environment
  - python -m virtualenv env
  - cd .\env\Scripts\
  - ./activate
- Execute the Python program using below commands. 
  - $env:FLASK_APP="app.py"
  - flask run

Outcome: Able to get the website running with mock data. 

Step 2: Running the migrations on original code. 
- Added the Migrate library and commands in app.py 
- run the commands below to create the basic table structure (without extra columns and Show table logic)
  - flask db init
  - flask db migrate
  - flask db upgrade

Outcome: Able to get the Venue and Artist Tables created in the fyyur database with the original columns

Step 3: Missing Columns, Show Table Creation and relationships addition via DB Migrate
- Update the python file app.py
  - Add the missing/new columns to Venue and Artist Model. 
  - Add Show table with the required relationships between Show, Venue and Artist table. 
- Run the migrate commands (used above) again to amend/create new columns and tables.

Outcome: All tables are updated with new Columns, Show Table created and relationships/foreign Key constraints added as required. 

Step 4: Insert the Raw data into tables using queries.txt file 
- Insert the data in the Venue/Artist/show Table as required using the queries given in the queries.txt file. 

Step 5: Implement Venue & Artist related functions as below. 
- Update the sections below with the python code by connecting to the database and populate the data related to various Venue/Artist functions as below.
  - List of Venue by City and State including their number of upcoming shows. 
  - List of Artists including their number of upcoming shows. Note: The number of upcoming shows are not being displayed via forms. 
  - Search a Venue/Artist (using Character insensitive Search function)
  - Listing the Artists detail about a selected Venue including the upcomings and past shows or vice-versa 
  - creating a new Venue/Artist
  - Deletng an existing Venue/Artist - No button created as of now to do the same. Only functionality is added.
  - Edit Venue/Artist Details 

Outcome: 
- All functions works as expected. 
- Only issue found was that the Create Venue/Artist/Show wasn't working in the first time. Somehow the Insert Statement used via PSQL has not updated the Primary ID counter for these tables. So after  failures depending on the number of rows in the tables it ran successfully. 

Step 5: Show Functions
- List of all shows
- Create a new show 